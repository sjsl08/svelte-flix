...
<main class="relative overflow-hidden ">
  ...
  {#if movie}
    <div class="absolute top-1/3 left-5 md:left-10 w-full z-10">
      <h1 class="text-4xl md:text-6xl font-bold mb-4 text-white">
        {movie.title || movie.name}
      </h1>
      <p class="text-sm md:text-lg mb-6 max-w-lg text-gray-300">
        {movieDetails ? movieDetails.overview : movie.overview}
      </p>
      <div class="flex flex-wrap items-center space-x-4 space-y-2 md:space-y-0">
        <!-- Play Button -->
        <button class="flex items-center gap-2 bg-white text-black px-4 py-2 rounded-md hover:bg-gray-200 transition">
          <Play size={20} />
